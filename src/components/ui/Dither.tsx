...
const int OCTAVES = 4;
float fbm(vec2 p, float freq, float amp_factor) {
  float value = 0.0;
  float amp = 1.0;
...
float pattern(vec2 p, float time, float waveSpeed, float waveFreq, float waveAmp) {
...
class RetroEffectImpl extends Effect {
  ...
  update(renderer: THREE.WebGLRenderer, inputBuffer: THREE.WebGLRenderTarget, deltaTime: number) {
    ...
    if (resolution.value.x !== inputBuffer.width || resolution.value.y !== inputBuffer.height) {
